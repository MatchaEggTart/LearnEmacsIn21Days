...
* 快捷键
  ...
  * 组合键
    >C-u  前缀参数\
    >&emsp;这个指令跟上数字后等于重复了多少次，再输入指令或输入，就是重复多少次指令或输入\
    >&emsp;例子 C-u 8 C-f 或者 C-u 10 *
  ...
  * 粘贴/召回(yank)
    >C-y  粘贴\
    >&emsp;如果遇上 C-u 30 C-k，C-y 也能把30行一次召回，因为同一指令下，都是可以召回\
    >M-y  召回 kill 缓冲区的内容\
    >&emsp;必须先使用 C-y，才能使用 M-y
  ...
  * 递归编辑(RECURSIVE EDITING LEVELS)
    >M-% 递归编辑\
    >&emsp;M-% 加上 C-s和C-r然后递归询问是否要切换这个字符串，\
    >&emsp;切换后再按 C-s或C-r会提示是否要继续切换
  ...
  * 获得更多帮助(GETTING MORE HELP)
    ...
    >&emsp;输入一个关键词后 Emacs 会列出所有命令名中包含此关键词的命令。\
  ...
  * 主要术语的译词对照，并给出注释说明：
    ...
    &emsp;mode line             状态栏\
    &emsp;search                搜索\
    &emsp;incremental search    渐进式搜索 [3]
